...
import com.sitexa.lovechain.modules.dapp.paidanswer.paidanswerhome.activity.PaidAnswerActivity;
...
import com.sitexa.lovechain.view.convenientbanner.ConvenientBanner;
import com.sitexa.lovechain.view.convenientbanner.adapter.LocalImageHolderView;
import com.sitexa.lovechain.view.convenientbanner.holder.CBViewHolderCreator;
import com.sitexa.lovechain.view.convenientbanner.listener.OnItemClickListener;
...
public class DappFragment extends BaseFragment<DappView, DappPresenter> implements DappView {
    ...
    private List<DappCompanyBean.DataBean> mHeaderList = new ArrayList<>(); //头部四个icon
    ...
    @Override
    public void getDappCompanyDataHttp(final List<DappCompanyBean.DataBean> dappCompanyBean) {

        if (dappCompanyBean.size() != 0 && dappCompanyBean.size() > 3) {
            mRecycleApplication.setVisibility(View.VISIBLE);
            for (int i = 0; i < 3; i++) {
                ivLsit.add(dappCompanyBean.get(i).getPublicImage());
            }
            ...
            mBanner.setOnItemClickListener(new OnItemClickListener() {
                ...
                public void onItemClick(int position) {
                    ...
                    bundle.putParcelable("bean", dappCompanyBean.get(position));
                    ActivityUtils.next(getActivity(), DappCompanyDetailsActivity.class, bundle);
                }
            });

            for (int i = 3; i < dappCompanyBean.size(); i++) {
                mHeaderList.add(dappCompanyBean.get(i));
            }
            mHeaderAdapter.notifyDataSetChanged();

        } else {
            for (int i = 0; i < dappCompanyBean.size(); i++) {
                ivLsit.add(dappCompanyBean.get(i).getPublicImage());
            }
            mBanner.setPages(new CBViewHolderCreator() {
                @Override
