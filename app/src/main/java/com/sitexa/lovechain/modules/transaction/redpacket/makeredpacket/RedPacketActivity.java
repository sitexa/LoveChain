...
import com.sitexa.lovechain.blockchain.EosDataManager;
...
import com.sitexa.lovechain.bean.TransferEosMessageBean;
...
import com.sitexa.lovechain.utils.RotateUtils;
...
import com.sitexa.lovechain.view.popupwindow.BasePopupWindow;
import com.sitexa.lovechain.view.textwatcher.MakeRedPacketMoneyTextWatcher;
import com.sitexa.lovechain.view.textwatcher.MakeRedPacketNumberTextWatcher;
...
public class RedPacketActivity extends BaseActivity<RedPacketView, RedPacketPresenter> implements RedPacketView {
    ...
    public void sendRedPacketDataHttp(SendRedPacketBean.DataBean sendRedPacketBean) {
        ...
        if (mSwitchProperty.getText().toString().equals("EOS")) {
            new EosDataManager(RedPacketActivity.this, userPassword).setCoinRate(coinRate).setRedpacketInfo(sendRedPacketBean, mRedPacketNumber.getText().toString().trim())
                    .sendRedPacket(
            ...
            new EosDataManager(RedPacketActivity.this, userPassword).setRedpacketInfo(sendRedPacketBean, mRedPacketNumber.getText().toString().trim())
                    .sendRedPacket(
                            new Gson().toJson(new TransferEosMessageBean(mLeaveMessage.getText().toString().trim()
                                    , "oc.redpacket",
                                    StringUtils.addZero(mRedPacketMoney.getText().toString().trim()) + " " + mSwitchProperty.getText().toString().trim(),
